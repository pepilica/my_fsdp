...
class FSDPParam:
    ...
    def to_sharded(self) -> None:
        self._setattr_on_module(self.sharded_param)
        free_storage(self.unsharded_param.data)
        self.sharded_state = ShardedState.SHARDED
    ...
    def to_unsharded(self) -> None:
        ...
        self._unsharded_param = nn.Parameter(
            self.unsharded_param.data.to(self.param_dtype),
    ...
    def unshard(self):
        ...
        with record_function(self.with_fqn("FSDP::all_gather")):
            for param in self.fsdp_params:
                alloc_storage(param.unsharded_param)
                dist.all_gather_into_tensor(
                    param.unsharded_param.data,
                    param.sharded_param.to(dtype=param.param_dtype).to_local(),
                    group=param.mesh.get_group()
                )

    def wait_for_unshard(self):
        for param in self.fsdp_params:
            param._setattr_on_module(param.unsharded_param)
        self._to_unsharded()
...
def post_backward(module: FSDPModule):
    ...
    with record_function(module.with_fqn("FSDP::post_backward_reduce")):
        ...
        for param in module.fsdp_params:
            unsharded_grad = param.unsharded_param.grad.to(param.reduce_dtype)
            sharded_grad = torch.zeros_like(param.sharded_param._local_tensor)
            dist.reduce_scatter_tensor(sharded_grad, unsharded_grad)
            param.sharded_param.grad = DTensor.from_local(unsharded_grad, param.mesh, ((param.fsdp_placement, ))).to(param.orig_dtype)
            free_storage(param.unsharded_param.grad)
